...
async fn exact_stuck_set() {
    ...
    let start = Instant::now(); let deadline = start + Duration::from_secs(90);
    loop {
        ...
        if all { let l=m.list().await;
            ...
            m.shutdown().await; let _=std::fs::remove_dir_all(&dir); assert!(ok==ids.len()); return;
        }
        if Instant::now()>deadline {
            ...
            m.shutdown().await; let _=std::fs::remove_dir_all(&dir); panic!("hang"); 
        }
